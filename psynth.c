 ...
 * that has to pass before a voice playing a given note has to toggle its
 * state.
 */
...
{
	bool bit = state.last_amp > 0.0;
	sample_t amp = bit? state.last_amp : -state.last_amp;
	...
	for (i = 0; i < nframes; i++) {
		...
		buffer[i] = bit? amp : -amp;
		//putchar('0'+bit);
	}

	//putchar('\n');

	state.last_amp = buffer[nframes-1];
}
